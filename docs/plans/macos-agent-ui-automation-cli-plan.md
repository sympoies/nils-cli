...
- **Location**:
  - `docs/adr/adr-macos-agent-reuse-screen-record.md`
...
- **Complexity**: 5
- **Acceptance criteria**:
  - ADR includes explicit keep/reuse/do-not-reuse decisions per module.
  - ADR documents coupling risks, CI implications, and migration path if extraction is needed later.
  - v1 decision is actionable without requiring an additional spike sprint.
- **Validation**:
  - `rg -n "reuse matrix|decision|types|select|shareable|screenshot|migration" docs/adr/adr-macos-agent-reuse-screen-record.md`

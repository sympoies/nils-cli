...
Source scripts (current): `https://github.com/graysurf/agent-kit/tree/main/skills/workflows/plan/plan-tooling/scripts`.
...
3. `AGENT_HOME` may be unset; the CLI derives repo root from its own location when running from a checked-out repo, matching the current scripts’ behavior.
...
- Repo root discovery: `AGENT_HOME` auto-detection must match current behavior to keep repo-relative paths stable in JSON output.

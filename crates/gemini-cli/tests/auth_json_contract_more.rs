...
fn auth_json_contract_save_missing_paths_are_structured() {
    ...
    let missing_secret_output = run(
        ...
        &[
            ...
            ("GEMINI_SECRET_DIR", &missing_secret_dir),
        ],
    );
    ...
    let missing_auth_output = run(
        ...
        &[
            ("GEMINI_SECRET_DIR", &secrets),
            ("GEMINI_AUTH_FILE", &missing_auth_path),
        ],
    );
...
fn auth_json_contract_current_not_configured_and_secret_dir_read_failed_are_structured() {
    ...
    let read_failed = run(
        ...
        &[
            ("GEMINI_AUTH_FILE", &auth_file),
            ("GEMINI_SECRET_DIR", &secret_dir_file),
        ],
    );
...
fn auth_json_contract_refresh_request_failure_is_structured() {
    ...
    let output = run_with(
        ...
        &[
            ...
            ("https_proxy", "http://127.0.0.1:9"),
            ("GEMINI_REFRESH_AUTH_CURL_CONNECT_TIMEOUT_SECONDS", "1"),
            ("GEMINI_REFRESH_AUTH_CURL_MAX_TIME_SECONDS", "1"),
        ],
    );

...
pub fn resolve_grpc_setup_dir_for_call(
    cwd: &Path,
    invocation_dir: &Path,
    request_file: &Path,
    config_dir: Option<&Path>,
    ...
    SetupDiscovery {
        ...
        seed_fallback: FallbackMode::None,
        invocation_dir: Some(invocation_dir),
        invocation_fallback: FallbackMode::Direct("setup/grpc", SETUP_GRPC_ERROR),
    }
    .resolve()
}

pub fn resolve_grpc_setup_dir_for_history(
    cwd: &Path,
    invocation_dir: &Path,
    config_dir: Option<&Path>,
) -> Result<PathBuf> {
...
mod tests {
    ...
    #[test]
    fn config_grpc_call_falls_back_to_setup_grpc_in_invocation_dir() {
        let tmp = TempDir::new().expect("tmp");
        let root = std::fs::canonicalize(tmp.path()).expect("root abs");

        write_file(
            &root.join("setup/grpc/endpoints.env"),
            "export GRPC_URL_LOCAL=127.0.0.1:50051\n",
        );
        write_file(
            &root.join("requests/health.grpc.json"),
            r#"{"method":"health.HealthService/Check","body":{}}"#,
        );

        let setup_dir = resolve_grpc_setup_dir_for_call(
            &root,
            &root,
            &root.join("requests/health.grpc.json"),
            None,
        ...
        assert_eq!(setup_dir, root.join("setup/grpc"));
    }

    #[test]
    fn resolved_setup_grpc_uses_grpc_history_default() {
        let tmp = TempDir::new().expect("tmp");
        let root = std::fs::canonicalize(tmp.path()).expect("root abs");

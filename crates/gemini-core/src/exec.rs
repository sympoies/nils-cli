...
pub fn require_allow_dangerous(caller: Option<&str>, stderr: &mut impl Write) -> bool {
    if is_true_env("GEMINI_ALLOW_DANGEROUS_ENABLED", stderr) {
        return true;
    }
    ...
    let _ = writeln!(
        stderr,
        "{prefix}: disabled (set GEMINI_ALLOW_DANGEROUS_ENABLED=true)"
    );
    false
}
...
pub fn exec_dangerous(prompt: &str, caller: &str, stderr: &mut impl Write) -> i32 {
    if prompt.is_empty() {
        let _ = writeln!(stderr, "_gemini_exec_dangerous: missing prompt");
        return 1;
    }
    ...
    match shared_process::run_status_inherit("gemini", &args) {
        ...
        Err(err) => {
            let _ = writeln!(stderr, "gemini-tools: failed to run gemini exec: {err}");
            1
        }
    }
}

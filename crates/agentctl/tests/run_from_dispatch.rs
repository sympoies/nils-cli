...
fn run_from_provider_and_diag_subcommands_dispatch_successfully() {
    ...
    let agent_home = StubBinDir::new();
    let agent_home_dir = agent_home.path().join("agent-home");
    std::fs::create_dir_all(&agent_home_dir).expect("create agent home");
    let agent_home_value = agent_home_dir.to_string_lossy().to_string();
    let _agent_home = EnvGuard::set(&lock, "AGENT_HOME", agent_home_value.as_str());
...
fn run_from_debug_and_workflow_subcommands_dispatch_successfully() {
    ...
    let agent_home_dir = repo.path().join("agent-home");
    std::fs::create_dir_all(&agent_home_dir).expect("create agent home");
    let agent_home_value = agent_home_dir.to_string_lossy().to_string();
    let _agent_home = EnvGuard::set(&lock, "AGENT_HOME", agent_home_value.as_str());

...
fn metadata_uses_normalized_provider_identity() {
    let adapter = GeminiProviderAdapter::new();
...
fn capabilities_disable_execute_when_binary_is_missing() {
    ...
    assert_eq!(
        ...
        Some("gemini binary is not available on PATH")
    );
}
...
fn capabilities_include_experimental_when_requested() {
    ...
    assert_eq!(
        execute.description.as_deref(),
        Some("gemini execution is enabled")
    );
...
fn healthcheck_is_unhealthy_when_binary_is_missing() {
    ...
    assert_eq!(
        ...
        Some("gemini adapter is unavailable")
    );
    let details = response.details.expect("details");
    assert_eq!(details["gemini_binary_available"], false);
...
fn healthcheck_is_degraded_when_policy_is_disabled() {
    ...
    assert_eq!(
        response.summary.as_deref(),
        Some("gemini adapter is partially ready")
    );
    let details = response.details.expect("details");
    assert_eq!(details["gemini_binary_available"], true);
...
fn healthcheck_is_healthy_when_policy_enabled_and_auth_file_exists() {
    ...
    assert_eq!(response.summary.as_deref(), Some("gemini adapter is ready"));
    let details = response.details.expect("details");
    assert_eq!(details["gemini_binary_available"], true);
...
fn execute_returns_validation_error_when_policy_is_disabled() {
    ...
    assert!(
        error
            .message
            .contains("GEMINI_ALLOW_DANGEROUS_ENABLED=true")
    );
}

#[test]
fn execute_returns_internal_error_when_gemini_exits_non_zero() {
    let lock = GlobalStateLock::new();
    let stub = StubBinDir::new();
